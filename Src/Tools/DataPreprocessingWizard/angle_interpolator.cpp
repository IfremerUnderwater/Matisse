...
double AngleInterpolator::findValue(double x) const {
  ...
  // delta Y modulo
  double max = M_PI*2;
  double deltaY = fmod(upperY - lowerY, max);
  double short_angle_dist= fmod(2*deltaY,max) - deltaY;

  return fmod(lowerY + short_angle_dist*(x - lowerX)/deltaX,max);
