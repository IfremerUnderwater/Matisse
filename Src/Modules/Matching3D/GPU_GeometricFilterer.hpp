...
#ifndef OPENMVG_MATCHING_IMAGE_COLLECTION_GPU_GEOMETRIC_FILTER_HPP
#define OPENMVG_MATCHING_IMAGE_COLLECTION_GPU_GEOMETRIC_FILTER_HPP
...
struct GpuImageCollectionGeometricFilter
{
  GpuImageCollectionGeometricFilter
  (
...
template<typename GeometryFunctor>
void GpuImageCollectionGeometricFilter::Robust_model_estimation
(
  const GeometryFunctor & functor,
  const PairWiseMatches & putative_matches,
  const bool b_guided_matching,
  const double d_distance_ratio,
  const bool use_sift_gpu,
...
#endif // OPENMVG_MATCHING_IMAGE_COLLECTION_GPU_GEOMETRIC_FILTER_HPP
