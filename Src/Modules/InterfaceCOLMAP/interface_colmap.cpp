...
Q_EXPORT_PLUGIN2(InterfaceColmap, InterfaceColmap)
#endif

namespace matisse {

	// This plugin is an adaptation from openMVS InterfaceCOLMAP
	//using namespace MVS;
	// D E F I N E S ///////////////////////////////////////////////////
...
// S T R U C T S ///////////////////////////////////////////////////
	...
	namespace COLMAP {
		...
		struct Point {
			...
			MVS::Interface::Pos3f p; // position
			MVS::Interface::Col3 c; // BGR color
			float e; // error
	...
	bool ImportScene(const String& strFolder, const String& strOutFolder, MVS::Interface& scene, MVS::PointCloud& pointcloud)
	{
		// read camera list
		...
		CamerasMap mapCameras;
		{
			...
			while (file.good() && colmapCamera.Read(file, binary)) {
				...
				if (OPT::bNormalizeIntrinsics) {
					...
					camera.K = MVS::Camera::ScaleK<double>(camera.K, 1.0 / MVS::Camera::GetNormalizationScale(colmapCamera.width, colmapCamera.height));
				}
				else {
		...
		const String filenameDenseVisPoints(strFolder + COLMAP_DENSE_POINTS_VISIBILITY);
		{
			...
			while (file.good() && point.Read(file, binary)) {
				...
				std::sort(vertex.views.begin(), vertex.views.end(),
					[](const MVS::Interface::Vertex::View& view0, const MVS::Interface::Vertex::View& view1) { return view0.imageID < view1.imageID; });
				scene.vertices.emplace_back(std::move(vertex));
				scene.verticesColor.emplace_back(MVS::Interface::Color{ point.c });
			}
		}
		pointcloud.Release();
		if (File::access(filenameDensePoints) && File::access(filenameDenseVisPoints)) {
			...
			for (size_t i = 0; i < numPoints; ++i) {
				MVS::PointCloud::ViewArr& views = pointcloud.pointViews[i];
				uint32_t numViews(0);
		...
		if (File::isFolder(pathDepthMaps) && File::isFolder(pathNormalMaps)) {
			...
			CLISTDEF2IDX(MVS::IIndexArr, MVS::IIndex) imagesNeighbors((MVS::IIndex)scene.images.size());
			{
				...
				while (true) {
					...
					if (it_image == mapImages.end())
						continue;
					MVS::IIndexArr& imageNeighbors = imagesNeighbors[it_image->second];
			...
			FOREACH(idx, scene.images) {
				const MVS::Interface::Image& image = scene.images[idx];
				...
				if (!colDepthMap.data_.empty()) {
					MVS::IIndexArr IDs = { image.ID };
					IDs.Join(imagesNeighbors[(MVS::IIndex)idx]);
					const MVS::Interface::Platform& platform = scene.platforms[image.platformID];
					const MVS::Interface::Platform::Pose pose(platform.GetPose(image.cameraID, image.poseID));
					const MVS::Interface::Mat33d K(platform.GetFullK(image.cameraID, (uint32_t)colDepthMap.width_, (uint32_t)colDepthMap.height_));
					...
					if (!MVS::ExportDepthDataRaw(strOutFolder + String::FormatString("depth%04u.dmap", image.ID), MAKE_PATH_FULL(strOutFolder, image.name), IDs, depthMap.size(), K, pose.R, pose.C, dMin, dMax, depthMap, normalMap, confMap, viewsMap))
						return false;
				}
			}
		}
	...
	bool ImportPointCloud(const String& strPointCloudFileName, MVS::Interface& scene)
	{
		...
		FOREACH(i, pointcloud.points) {
			...
			FOREACH(j, pointcloud.pointViews[i]) {
				MVS::Interface::Vertex::View& view = vertex.views.emplace_back();
			...
			if (!pointcloud.colors.empty()) {
				...
				scene.verticesColor.emplace_back(MVS::Interface::Color{ MVS::Interface::Col3{c.b, c.g, c.r} });
			}
			if (!pointcloud.normals.empty())
				scene.verticesNormal.emplace_back(MVS::Interface::Normal{ pointcloud.normals[i] });
		}
		return true;
	}

	bool ExportScene(const String& strFolder, const MVS::Interface& scene, bool bForceSparsePointCloud = false, bool binary = true)
	{
		...
		CLISTDEF0IDX(COLMAP::Camera, uint32_t) cams;
		{
			...
			if (binary) {
				cam.numCameras = 0;
				for (const MVS::Interface::Platform& platform : scene.platforms)
			...
			for (uint32_t ID = 0; ID < (uint32_t)scene.platforms.size(); ++ID) {
				const MVS::Interface::Platform& platform = scene.platforms[ID];
				ASSERT(platform.cameras.size() == 1); // only one camera per platform supported
				const MVS::Interface::Platform::Camera& camera = platform.cameras[0];
				cam.ID = ID;
				KMatrix K;
				if (camera.width == 0 || camera.height == 0) {
					// find one image using this camera
					const MVS::Interface::Image* pImage(NULL);
					for (uint32_t i = 0; i < (uint32_t)scene.images.size(); ++i) {
						const MVS::Interface::Image& image = scene.images[i];
					...
					IMAGEPTR ptrImage(MVS::Image::ReadImageHeader(MAKE_PATH_SAFE(pImage->name)));
					if (ptrImage == NULL)
						return false;
		...
		const uint32_t avgPointsPerSmallView(3000), avgPointsPerLargeView(12000);
		{
			...
			for (uint32_t ID = 0; ID < (uint32_t)scene.images.size(); ++ID) {
				const MVS::Interface::Image& image = scene.images[ID];
				if (image.poseID == NO_ID)
					continue;
				const MVS::Interface::Platform& platform = scene.platforms[image.platformID];
				const MVS::Interface::Platform::Pose& pose = platform.poses[image.poseID];
		...
		if (bSparsePointCloud || bForceSparsePointCloud) {
			// write points list
			{
				...
				for (uint32_t ID = 0; ID < (uint32_t)scene.vertices.size(); ++ID) {
					const MVS::Interface::Vertex& vertex = scene.vertices[ID];
					COLMAP::Point point;
					point.ID = ID;
					point.p = vertex.X;
					for (const MVS::Interface::Vertex::View& view : vertex.views) {
					...
					point.c = scene.verticesColor.empty() ? MVS::Interface::Col3(255, 255, 255) : scene.verticesColor[ID].c;
					point.e = 0;
					if (numPoints3D != 0) {
		...
		if (!bSparsePointCloud) {
			...
			MVS::PointCloud pointcloud;
			for (size_t i = 0; i < numPoints; ++i) {
				const MVS::Interface::Vertex& vertex = scene.vertices[i];
				...
				for (uint32_t v = 0; v < numViews; ++v) {
					const MVS::Interface::Vertex::View& view = vertex.views[v];
	...
	bool ExportIntrinsicsTxt(const String& fileName, const MVS::Interface& scene)
	{
		...
		if (idxValidK == NO_ID)
			return false;
		const MVS::Interface::Image& image = scene.images[idxValidK];
		...
		IMAGEPTR pImage = MVS::Image::ReadImageHeader(imagefileName);
		if (!pImage) {
			VERBOSE("error: unable to open image file '%s'", imagefileName.c_str());
			return false;
		}
		const MVS::Interface::Mat33d& K = scene.platforms[image.platformID].GetFullK(image.cameraID, pImage->GetWidth(), pImage->GetHeight());
	...
	bool ExportImagesLog(const String& fileName, const MVS::Interface& scene)
	{
		...
		out << std::setprecision(12);
		MVS::IIndexArr orderedImages((uint32_t)scene.images.size());
		std::iota(orderedImages.begin(), orderedImages.end(), 0u);
		orderedImages.Sort([&scene](MVS::IIndex i, MVS::IIndex j) {
			return scene.images[i].ID < scene.images[j].ID;
			});
		for (MVS::IIndex ID : orderedImages) {
			const MVS::Interface::Image& image = scene.images[ID];
	...
	bool ExportImagesCamera(const String& pathName, const MVS::Interface& scene)
	{
		...
		for (uint32_t ID = 0; ID < (uint32_t)scene.images.size(); ++ID) {
			...
			if (image.platformID != NO_ID && image.cameraID != NO_ID) {
				const MVS::Interface::Platform& platform = scene.platforms[image.platformID];
				const MVS::Interface::Platform::Camera& camera = platform.cameras[image.cameraID];
				if (camera.HasResolution()) {
					...
					IMAGEPTR pImage = MVS::Image::ReadImageHeader(image.name);
	...
	// Real plugin begins here

	InterfaceColmap::InterfaceColmap() :
		Processor(NULL, "InterfaceColmap", "Colmap to OpenMVS interface", 1, 1)
	{
	...
	bool InterfaceColmap::configure()
	{
		return true;
	}

	void InterfaceColmap::onNewImage(quint32 _port, Image& _image)
	{
		Q_UNUSED(_port)
	...
	bool InterfaceColmap::start()
	{
		setOkStatus();

		std::cerr << "***********Start colmap interface" << std::endl;
		return true;
	}

	bool InterfaceColmap::stop()
	{
		std::cerr << "\n***********Stop colmap interface" << std::endl;
		return true;
	}

	// initialize and parse the command line parameters
	bool InterfaceColmap::Initialize()
	{
	...
	void InterfaceColmap::onFlush(quint32 _port)
	{
		QElapsedTimer timer;
		timer.start();
		...
		if (object)
		{
			...
			if (rc->current_format != ReconFormat::colmap)
			{
				fatalErrorExit("Only colmap format is expected as input !");
				return;
			}
		}
		else
		...
		for (int i : rc->components_ids)
		{
			...
			{
					fatalErrorExit("Missing a sparse folder from colmap");
					return;
			}

			// init variables
			OPT::strInputFileName = openmvs_dir.toStdString(); // input COLMAP folder containing cameras, images and points files OR input MVS project file
			OPT::strOutputFileName = OPT::strInputFileName + _T("scene") MVS_EXT; // output filename for storing the MVS project
			OPT::strImageFolder = openmvs_qdir.absoluteFilePath("images").toStdString() + qsep.toStdString(); // folder to the undistorted images	
			...
			{
				fatalErrorExit("Cannot import scene in openMVS");
				return;
			}
			// write MVS input data
			Util::ensureFolder(strOutFolder);
			if (!MVS::ARCHIVE::SerializeSave(scene, MAKE_PATH_SAFE(OPT::strOutputFileName)))
			{
				fatalErrorExit("Cannot save scene in openMVS format");
				return;
			}
			if (!pointcloud.IsEmpty() && !pointcloud.Save(MAKE_PATH_SAFE(Util::getFileFullName(OPT::strOutputFileName)) + _T(".ply"), true))
			{
				fatalErrorExit("Cannot save pointcloud in ply format");
				return;
			}
