...
//void SftpClient::sl_upload(QString _local_path, QString _remote_path, bool _is_dir_upload, bool _recurse)
//{
...
//    qDebug() << QString("SftpClient: uploading %1 files for a total of %2 bytes...").arg(file_count).arg(transfer_size);
...
//                    qDebug() << QString("SftpClient: cleaning temp dir '%1'").arg(temp_root_dir_path);
//                    bool removed = temp_root_dir.removeRecursively();
//                    if (!removed) {
//                        qWarning() << QString("SftpClient: could not create temp dir '%1'").arg(temp_root_dir_path);
//                    }
//                }
//                QString temp_dir_path = temp_root_dir_path + QDir::separator() + local_dir.dirName();
//                qDebug() << QString("SftpClient: creating temp dir '%1'").arg(temp_dir_path);
//                QDir temp_dir(temp_dir_path);
//                bool created = temp_dir.mkpath(".");
//                if (!created) {
//                    qWarning() << QString("SftpClient: could not create temp dir '%1'").arg(temp_dir_path);
//                }
...
//                if (!dir_copied) {
//                    qCritical() << QString("SFTP Client: copying source to local temp dir '%1' failed, upload aborted");
//                    return;
//                }
...
//        qDebug() << tr("SFTP Client: Uploading dir %1 to %2 ...")
//                    .arg(local_dir_path)
//                    .arg(_remote_path);
...
//        qDebug() << "SFTP Client upload: Starting job #" << job;
//    } else {
//        qCritical() << "SFTP Client upload: Invalid Job";
//    }
//}
...
//        sl_dirContent(_remote_path, eFileTypeFilter::Files, QStringList());
...
//void SftpClient::doInitBeforeAction() {
...
//    qDebug() << "SFTP Client: Processing SFTP action of type " << m_current_action->type();
...
//void SftpClient::reinitProgressIndicators(quint64 _transfer_size) {
//    //    qDebug() << QString("SftpClient: reinit progress indicators:");
...
//        if (m_current_action->type() == NetworkAction::eNetworkActionType::DownloadDir) {
...
//            qDebug() << QString("SftpClient: downloading %1 files for a total of %2 bytes").arg(file_count).arg(transfer_size);
...
//    //    qDebug() << QString("SFTP Client: transfer job %1 progress %2 out of %3 bytes")
//    //                    .arg(_job)
//    //                    .arg(_progress)
//    //                    .arg(_total);

//    if (m_current_transfer_size == 0) {
//        if (m_current_action->type() == NetworkAction::eNetworkActionType::DownloadFile) {
...
//    }
//    //    qDebug() << QString("SftpClient: prev progress: %1").arg(prev_progress);
...
//    //    qDebug() << QString("SftpClient: raw progress indicators:");
...
//    qWarning() << "SFTP error occurred : " << m_current_tx_error;
//}
